...
HITROST OCR: VEČ VZPOREDNIH WORKER-JEV (od v2.9)
---------------------------------------------------
BISTVO: če imate veliko slik (stotine/tisoče), v orodni vrstici zvišajte izbirnik
"X worker-jev" (ob izbirniku "OCR: Hitro/Natančno") na višjo vrednost PRED klikom
na "Ponovno preglej" — pregled bo hitrejši, ker se več slik OCR-a HKRATI namesto
ena za drugo.

Podrobnosti: do vključno v2.8 je aplikacija slike OCR-ala STROGO ZAPOREDNO (ena za
drugo), kar uporablja samo EN worker (torej efektivno eno jedro procesorja), ne
glede na to, koliko jih ima vaš računalnik na voljo. Od v2.9 lahko teče več
neodvisnih worker-jev hkrati, ki si RAZDELIJO seznam slik za obdelavo.
- Izbirnik je ob prvem odprtju vsakič PRIVZETO nastavljen glede na število jeder
  VAŠEGA procesorja (torej se lahko razlikuje med računalniki) — vrednosti od 1
  (enako kot prej, brez vzporednosti) do 8.
- KOMPROMIS: več worker-jev = hitreje, a tudi VEČ porabljenega RAM-a (vsak worker
  v pomnilniku nosi svojo lastno kopijo OCR mehanizma in jezikovnih podatkov). Če
  na svojem računalniku med pregledom opazite splošno upočasnitev sistema
  (ne le aplikacije), izbirnik znižajte na manjšo vrednost.
- Nastavitev NI trajno shranjena med sejami — vsak zagon aplikacije jo znova
  oceni glede na TA konkreten računalnik, na katerem jo ravno odpirate.
- V internem testu (majhen vzorec, v razvojnem/testnem okolju z omejenim številom
  jeder) je pregled s 4 worker-ji namesto 1 potekel v ~66 % prvotnega časa —
  dejanska pohitritev na vašem računalniku in vašem naboru slik je lahko
  drugačna (odvisna od števila jeder, RAM-a in velikosti slik).

VEČ MAP HKRATI ("+ Dodaj mapo", od v2.9)
--------------------------------------------
BISTVO: če želite iskati/OCR-ati po VEČ mapah hkrati (ne samo eni), kliknite gumb
"+ Dodaj mapo" (pod vrstico z imenom trenutne mape) in izberite dodatno mapo —
njena vsebina se takoj pridruži isti tabeli/iskanju, seznam dodanih map pa ostane
zapomnjen tudi po zaprtju brskalnika.

Podrobnosti:
- Gumb "+ Dodaj mapo" je omogočen šele, ko imate izbrano PRVOTNO (glavno) mapo
  (gumb "Izberi mapo…"). Vsaka dodana mapa se prikaže kot majhna "kapsula"
  (chip) z imenom mape ob gumbu — klik na "×" na njej odstrani mapo iz prikaza
  (SAME slike in njen "ocr-index.json" v tisti mapi ostanejo popolnoma
  nedotaknjeni — odstrani se samo iz TE aplikacije).
- Vsaka mapa (prvotna in vsaka dodatna) obdrži SVOJ LASTEN "ocr-index.json",
  zapisan nazaj v SVOJO mapo — tako lahko isto dodatno mapo po potrebi uporablja
  tudi kdo drug neodvisno, brez konflikta z vašim pregledom.
- V stolpcu "Mapa" v tabeli je za slike iz DODATNIH map pred podmapo dodano tudi
  ime njihove korenske mape (npr. "DrugaMapa/podmapa"), da je jasno razvidno, iz
  katere mape posamezna vrstica izvira. Za prvotno (glavno) mapo je prikaz
  nespremenjen.
- Ob "Izberi mapo…" (izbira NOVE prvotne mape) se seznam dodatnih map iz
  prejšnje mape POBRIŠE (verjetno ne pripadajo vsebinsko novi mapi) — dodajte
  jih po potrebi znova. Ob "Odpri zadnjo mapo" (nadaljevanje prejšnje seje) se
  prej dodane mape SAMODEJNO POSKUSIJO OBNOVITI (brskalnik za vsako lahko znova
  vpraša za dovoljenje — enaka varnostna zahteva kot pri sami prvotni mapi).
- Če je dovoljenje za katero od dodanih map zavrnjeno ali mapa ni več dosegljiva
  (premaknjena/izbrisana/na izklopljenem omrežnem disku), ta ENA mapa preprosto
  ni vključena v pregled (z opozorilom v gumbu "Dnevnik") — ostale mape
  (vključno s prvotno) se obdelajo naprej normalno.
