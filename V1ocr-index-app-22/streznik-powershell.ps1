...
# Popravek (2026-09-24): Samodejno odpiranje brskalnika po zagonu streznika,
# po vzoru ostalih zaganjalnikov (zazeni-macos.command, zazeni-windows.bat).
# Najprej poskusimo Microsoft Edge, pri neuspehu pa se vrnemo na privzet brskalnik.
# POMEMBNO: namenoma NE uporabljamo "Get-Command msedge" za preverjanje, ali Edge obstaja -
# msedge.exe na Windows praviloma NI na PATH, ampak je najdljiv prek registrskega vnosa
# "App Paths" (enak mehanizem, na katerega se v zazeni-windows.bat opira "where msedge" -
# glej opombo tam). "Get-Command" ta registrski mehanizem nezanesljivo/nedosledno razreši
# (odvisno od razlicice PowerShell), medtem ko "Start-Process" pri iskanju programa uporablja
# enako razresevanje kot Windows Raziskovalec/ShellExecute, ki App Paths VEDNO upošteva - zato
# neposredno POSKUSIMO "Start-Process msedge" in šele ob dejanski izjemi (Edge res ni najden)
# preidemo na privzet brskalnik.
$Url = "http://127.0.0.1:$Port/index.html"
try {
    Start-Process msedge -ArgumentList $Url -ErrorAction Stop
} catch {
    Write-Host "(Microsoft Edge ni bil najden - odpiram v privzetem brskalniku namesto tega.)"
    Start-Process $Url
}
